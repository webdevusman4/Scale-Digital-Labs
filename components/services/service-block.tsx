...
export function ServiceBlock({
  ...
  // Cluster-based logic (ambient glow removed)
  return (
          ...
          >
            ...
            <div className="relative z-10 flex flex-col gap-5">
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full border border-white/10 hover:border-white/25 bg-white/5 hover:bg-white/10 transition-all text-[15px] font-semibold text-transparent bg-clip-text bg-gradient-to-r from-[#7B2FF7] via-[#F72585] to-[#FF8C42] mt-4 group w-fit"
              >
                Start a Project
...
const SERVICES_DATA: Omit<ServiceBlockProps, 'index'>[] = [
  ...
  },
  {
    ...
    description: 'High-performance web architecture built for speed and security. We handle the entire lifecycle in-house, eliminating the need for you to juggle multiple agencies.',
    deliverables: [
  ...
  },
  {
    ...
    description: 'Data-driven campaigns that turn attention into revenue. We build a dominant brand presence that compounds trust and authority over time.',
    deliverables: [
...
function ClusterDivider({ label }: { label: string }) {
  const content = (
    {
      'BUILD & POWER': {
        headline: 'The Foundation You Build On',
        description: 'Development and infrastructure that powers everything else.',
      },
      'MARKET & GROW': {
        headline: 'The Engine That Drives Growth',
        description: 'Marketing and outreach that turns your platform into revenue.',
      },
    } as Record<string, { headline: string; description: string }>
  )[label];

  return (
    <div className="w-full max-w-[1200px] mx-auto px-6 md:px-12 flex flex-col items-center pt-20 pb-8 text-center">
      <div className="w-px h-16 bg-gradient-to-b from-transparent to-white/20 mb-6" />
      <div className="px-4 py-1.5 rounded-full border border-white/10 bg-white/[0.02] backdrop-blur-sm mb-4">
      ...
      {content && (
        <>
          ...
          <p className="text-white/60 text-[15px] max-w-[440px]">
            {content.description}
          </p>
        </>
      )}
